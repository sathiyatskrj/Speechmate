...
class _CameraTranslationScreenState
    extends ConsumerState<CameraTranslationScreen> with WidgetsBindingObserver {
  ...
  Future<void> _processCameraImage(CameraImage image) async {
    ...
    try {
      ...
      try {
        ...
        debugPrint("Text recognizer error: $e");
      }
      ...
      for (DetectedObject obj in objects) {
        ...
        if (objectName.isNotEmpty && objectConfidence > 0.5) {
          try {
            ...
            debugPrint("Predict object error: $e");
          }
        }
      }
      ...
      if (recognizedText != null) {
        for (TextBlock block in recognizedText.blocks) {
          ...
          if (blockText.isNotEmpty && blockText.length > 1) {
            try {
              final result =
                  await ref.read(neuralEngineProvider).predict(blockText);
              ...
              debugPrint("Predict text error: $e");
            }
          }
        }
      }
      ...
      debugPrint("Live OCR overall error: $e");
    }
    _isDetecting = false;
  }
  ...
  Future<void> _processStaticImage(String path) async {
    ...
    try {
      ...
      if (recognizedText.text.trim().isNotEmpty) {
        ...
        for (TextBlock block in recognizedText.blocks) {
          for (TextLine line in block.lines) {
            ...
            if (originalText.isNotEmpty) {
              final result =
                  await ref.read(neuralEngineProvider).predict(originalText);
              results.add({
        ...
        setState(() {
          _detectedObject =
              objectName.isNotEmpty ? "Also detected: $objectName" : "";
        ...
        setState(() {
          _detectedObject =
              "Detected Object: $objectName (${(objectConfidence * 100).toStringAsFixed(0)}%)";
          _translationResults = [
            {
              "original": objectName,
              "translation": result.text.isNotEmpty
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('No text or objects recognized. Try again.')),
          );
          setState(() {
    ...
    try {
      ...
      if (_cameraController!.value.isStreamingImages) {
        try {
          ...
          await Future.delayed(const Duration(
              milliseconds: 200)); // Allow camera hardware to settle
        } catch (_) {}
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Camera capture failed. Try using the Gallery instead.')),
        );
      }
                          ...
                          if (_allRecognizedText.isNotEmpty)
                            Padding(
                              ...
                              child: OutlinedButton.icon(
                                ...
                                label: const Text('Copy All Recognized Text',
                                    style: TextStyle(
                                ...
                                onPressed: () {
                                  Clipboard.setData(
                                      ClipboardData(text: _allRecognizedText));
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                        content:
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Column(
                                  ...
                                  children: [
                                    ...
                                    const Text("Nicobarese Translation",
                                        style: TextStyle(
                                            color: Colors.cyanAccent,
                                            fontSize: 12)),
                                    const SizedBox(height: 4),
          ...
          children: [
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Text("Capture",
                            style: TextStyle(
                                color:
                                    !_isLiveMode ? Colors.black : Colors.white,
                                fontWeight: FontWeight.bold)),
                      ),
                    ),
                    GestureDetector(
            ...
            if (!_isLiveMode && _capturedImagePath == null)
              Positioned(
                ...
                left: 0,
                right: 0,
                child: const Text("Align text within the box",
