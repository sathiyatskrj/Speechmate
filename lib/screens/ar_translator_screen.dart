...
class _ARTranslatorScreenState extends ConsumerState<ARTranslatorScreen>
    ...
    try {
      ...
      if (_cameraController != null) {
        try {
          ...
          debugPrint('Silent error caught: $e');
        }
      ...
      try {
        ...
        if (Platform.isAndroid) {
          debugPrint('[AR] Retrying camera init with ResolutionPreset.high...');
      ...
      try {
        ...
        debugPrint('[AR] Zoom query failed: $e');
      }
  ...
  Future<void> _onFrame(CameraImage image) async {
    ...
    if (elapsed > 2000) {
      ...
      if (_currentFps < 15 && _throttleMs < 1000) {
        ...
        debugPrint(
            '[AR] FPS low (${_currentFps.toStringAsFixed(1)}), throttle -> ${_throttleMs}ms');
    ...
    try {
      ...
      if (_lensMode == 0 || _lensMode == 1) {
        try {
          ...
          try {
            detectedLabels = await _imageLabeler.processImage(inputImage);
          } catch (e) {
            debugPrint("Image labeler error: $e");
          }
          ...
          String? bestPreciseLabel =
              preciseLabels.isNotEmpty ? preciseLabels.first.key : null;
          double bestConf =
              preciseLabels.isNotEmpty ? preciseLabels.first.value : 0.0;
          ...
          for (final obj in detectedObjects) {
            ...
            if (obj.labels.isNotEmpty) {
              ...
              if (!_isUselessLabel(rawBroad)) {
                ...
                if (bestPreciseLabel == null ||
                    broadLabel.confidence > bestConf) {
                  finalLabel = rawBroad;
          ...
          _speakTimer = Timer(const Duration(milliseconds: 1500), () {
            if (!_isPaused && mounted) {
              ref
                  ...
                  .speakNicobarese(top.nicobarese, englishWord: top.english);
            }
          });
        }
      }
  ...
  void _togglePause() async {
    ...
    if (_isPaused && _cameraController != null) {
      try {
                        ...
                        Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              Text(
                                  'Detection History ($_totalDetections total)',
                                  style: const TextStyle(
                                      color: Colors.white,
                                      fontSize: 16,
                                      fontWeight: FontWeight.bold)),
  ...
  Widget _buildBootScreen() => Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          TextButton.icon(
            ...
            label:
                const Text('Retry', style: TextStyle(color: Colors.cyanAccent)),
          ),
        ]),
      );
              ...
              builder: (_, __) => Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: _isPaused
                      ? Colors.white38
                      : Color.lerp(
                          Colors.green, Colors.cyanAccent, _pulseAnim.value)!,
                ),
              ),
            ),
            const SizedBox(width: 4),
    ...
    return Column(
      ...
      children: _items.take(3).toList().asMap().entries.map((e) {
        ...
        return GestureDetector(
          onTap: () => ref
              .read(ttsServiceProvider)
              .speakNicobarese(item.nicobarese, englishWord: item.english),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        color:
                            item.hasTranslation ? Colors.white : Colors.white38,
                        fontSize: i == 0 ? 26 : 20,
                        fontWeight: FontWeight.bold,
                      ),
                    ),
      ...
      child: Row(
        ...
        children: [
          ...
          _BottomBtn(
            ...
            label:
                _lensMode == 0 ? 'Auto' : (_lensMode == 1 ? 'Objects' : 'Text'),
            onTap: () => setState(() {
            ...
            color: _lensMode == 0
                ? Colors.cyanAccent
                : (_lensMode == 1 ? Colors.orangeAccent : Colors.purpleAccent),
          ),
          // Live indicator dot
          Column(children: [
            AnimatedBuilder(
        ...
        child: Column(children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              shape: BoxShape.circle,
              border: Border.all(
                  color: color?.withValues(alpha: 0.5) ?? Colors.white24),
            ),
...
class _BoxOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < items.length; i++) {
      ...
      canvas.drawPath(
          Path()
            ..moveTo(scaled.left, scaled.top + cornerLength)
            ..lineTo(scaled.left, scaled.top)
            ..lineTo(scaled.left + cornerLength, scaled.top),
          bracketPaint);
      canvas.drawPath(
          Path()
            ..moveTo(scaled.right - cornerLength, scaled.top)
            ..lineTo(scaled.right, scaled.top)
            ..lineTo(scaled.right, scaled.top + cornerLength),
          bracketPaint);
      canvas.drawPath(
          Path()
            ..moveTo(scaled.left, scaled.bottom - cornerLength)
            ..lineTo(scaled.left, scaled.bottom)
            ..lineTo(scaled.left + cornerLength, scaled.bottom),
          bracketPaint);
      canvas.drawPath(
          Path()
            ..moveTo(scaled.right, scaled.bottom - cornerLength)
            ..lineTo(scaled.right, scaled.bottom)
            ..lineTo(scaled.right - cornerLength, scaled.bottom),
          bracketPaint);
      ...
      canvas.drawLine(Offset(center.dx - 10, center.dy),
          Offset(center.dx + 10, center.dy), crosshairPaint);
      canvas.drawLine(Offset(center.dx, center.dy - 10),
          Offset(center.dx, center.dy + 10), crosshairPaint);
      ...
      final subTp = TextPainter(
        text: TextSpan(
          text:
              ' ${item.english.toUpperCase()} [${(item.confidence * 100).toInt()}%] ',
          style: TextStyle(
              color: color,
              fontSize: i == 0 ? 11 : 9,
      ...
      final double panelWidth =
          math.max(math.max(tp.width, subTp.width), maxInfoWidth) + 16;
      final double panelHeight =
          tp.height + subTp.height + totalInfoHeight + 16;

      final badgeRect = Rect.fromLTWH(
        scaled.right +
            20, // Draw on right side to look like a real scanning HUD
...
class _ScanFramePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Trail gradient
    final gradRect = Rect.fromLTRB(rect.left, sy - 40, rect.right, sy);
    canvas.drawRect(
      gradRect,
      Paint()
