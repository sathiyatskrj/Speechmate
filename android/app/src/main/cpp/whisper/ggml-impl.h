...
struct ggml_hash_set {
  ...
  struct ggml_tensor **keys; // actual tensors in the set, keys[i] is only
                             // defined if ggml_bitset_get(used, i)
};
...
// returns GGML_HASHSET_FULL if table is full, otherwise the current index of
...
// returns GGML_HASHSET_ALREADY_EXISTS if key already exists, index otherwise,
...
struct ggml_cgraph {
  ...
  struct ggml_tensor *
      *nodes; // tensors with data that can change if the graph is evaluated
  struct ggml_tensor *
      *grads; // the outputs of these tensors are the gradients of the nodes
  struct ggml_tensor **grad_accs; // accumulators for node gradients
  struct ggml_tensor **leafs;     // tensors with constant data
  int32_t
      *use_counts; // number of uses of each tensor, indexed by hash table slot
...
static inline float ggml_e8m0_to_fp32(uint8_t x) {
  ...
  float result; // Final float value
                // Safely reinterpret bit pattern as float without type-punning
                // issues
...
static inline int32_t ggml_node_get_use_count(const struct ggml_cgraph *cgraph,
                                              int node_idx) {
...
static inline bool ggml_node_has_n_uses(const struct ggml_cgraph *cgraph,
                                        int node_idx, int32_t n_uses) {
  const struct ggml_tensor *node = cgraph->nodes[node_idx];
...
// Returns true if nodes with indices { node_idxs } are the sequence of ggml_ops
// in ops[] and are fusable. Nodes are considered fusable according to this
// function if:
// - all nodes except the last have only one use and are not views/outputs (see
// ggml_node_has_N_uses).
// - all nodes except the last are a src of the following node.
// - all nodes are the same shape.
// TODO: Consider allowing GGML_OP_NONE nodes in between
static inline bool ggml_can_fuse_ext(const struct ggml_cgraph *cgraph,
                                     const int *node_idxs,
                                     const enum ggml_op *ops, int num_ops) {
...
static inline bool ggml_can_fuse(const struct ggml_cgraph *cgraph, int node_idx,
                                 const enum ggml_op *ops, int num_ops) {
  assert(num_ops < 32);
...
static inline bool ggml_can_fuse_subgraph(const struct ggml_cgraph *cgraph,
  ...
  return ggml_can_fuse_subgraph_ext(cgraph, idxs, count, ops, outputs,
                                    num_outputs);
}
...
inline bool ggml_can_fuse_subgraph(const struct ggml_cgraph *cgraph,
  ...
  return ggml_can_fuse_subgraph(cgraph, start_idx, ops.size(), ops.begin(),
                                outputs.begin(), outputs.size());
}

// Return true if the edges in the graph match expectations.
inline bool ggml_check_edges(const struct ggml_cgraph *cgraph, int start_idx,
                             std::initializer_list<std::array<int, 3>> edges) {
  for (const auto &edge : edges) {
    int dst_node = edge[0];
    int src_idx = edge[1];
    int src_node = edge[2];
    if (cgraph->nodes[start_idx + dst_node]->src[src_idx] !=
        cgraph->nodes[start_idx + src_node]) {
      return false;
    }
  }
  return true;
...
gguf_init_from_file_impl(FILE *file, struct gguf_init_params params);
GGML_API void gguf_write_to_buf(const struct gguf_context *ctx,
                                std::vector<int8_t> &buf, bool only_meta);
#endif // __cplusplus
